...
## 2026-09-19：URL 回调消息接收修复与诊断日志

修复了 URL 模式错误依赖企业微信 BotID 的问题：旧版本要求手填 BotID，并将回调消息的 `aibotid` 与它比较，不一致便拒绝接收，可能出现“回调地址验证成功，但聊天消息没有入库”。现在 URL 模式只需配置 Token 和 EncodingAESKey，仍执行签名及解密校验；长连接模式继续要求 BotID 和 Secret，并保留身份检查。前端表单、机器人列表和接入说明同步调整，已有 URL 机器人无需重建或迁移数据库。

回调地址必须从控制台复制完整内容，例如 `https://bot.example.com/callbacks/wecom?bot=内部记录ID`。这里的 `bot` 是本系统用于选择机器人配置的记录 ID，不是企业微信 BotID。缺少该参数、机器人未启用或模式不是 URL，应用均返回 404。

新增默认开启的 JSON 诊断日志，为每次回调生成 `requestId`，并通过响应头 `X-Request-ID` 返回。日志覆盖请求到达、机器人匹配、签名解密、消息解析、缺少消息 ID 而跳过、重复消息去重、入库、AI 入队或跳过原因，以及最终 HTTP 状态和耗时。异常记录处理阶段、已知错误原因和调用栈位置；不打印凭证、完整查询串、加密报文、消息正文或回复凭证。

```bash
pm2 logs wecom-bridge --lines 200
```

单聊发送文本后，沿同一 `requestId` 检查：

| 日志 | 含义 |
| --- | --- |
| `callback.received` | 请求已经到达应用 |
| `callback.rejected` | 查看 `reason`，检查 bot 参数、启用状态及模式 |
| `callback.failed` | 查看 `stage`、`reason` 和 `stack`，定位解析、解密或入库处理失败 |
| `message.skipped` | 缺少字符串类型的消息 ID，未入库 |
| `message.stored` / `message.duplicate` | 已入库 / 重复投递未新增 |
| `message.ai_queue` / `message.ai_skipped` | 自动回复入队结果 / 跳过原因 |
| `callback.finished` | 最终状态码和耗时 |

验证：6 项自动化测试通过，涵盖单聊文本入库、URL 模式无需 BotID、回调去重、失败阶段日志和日志脱敏；生产前端构建通过。本次部署后用户已反馈消息接收恢复正常。

更新时覆盖应用代码和构建好的 `dist`，保留服务器 `.env` 与 `data/`，执行 `npm ci --omit=dev` 后重启应用；不要删除整个项目目录或重新运行初始化。完整操作及日志事件列表见[部署手册的回调诊断章节](docs/deployment.md#回调诊断与本次覆盖部署)。诊断日志默认持续输出，生产环境请配置日志轮转。
...
| 机器人 | 创建、停用后编辑、启停、回调地址复制；长连接 BotID 全局唯一 |
...
2. URL 模式填写 Token、EncodingAESKey，将生成的完整回调地址填入企业微信并验证；长连接模式填写 BotID 和专用 Secret，等待显示“已连接”。
